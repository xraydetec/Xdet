...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes,sample_type='fpn', k0 = 4):
    ...
    fg_inds = np.round(fg_inds).astype(np.int)
    # Guard against the case when an image has fewer than fg_rois_per_image
    # foreground RoIs
    fg_rois_per_this_image = min(fg_rois_per_image, fg_inds.size)
    fg_rois_per_this_image = np.round(fg_rois_per_this_image).astype(np.int)
    # Sample foreground regions without replacement
    if fg_inds.size > 0:
        fg_inds = npr.choice(fg_inds, size=fg_rois_per_this_image, replace=False)

    #find  all no foreground indices
    nonfg_inds = np.where(max_overlaps < cfg.TRAIN.FG_THRESH)[0]
    ...
    bg_rois_per_this_image = np.round(bg_rois_per_this_image).astype(np.int)

    #how many top score bg regions are chosen
    ratio = 0.25 
    bg_highscor_this_image =  int( bg_rois_per_this_image*ratio )
    bg_random_this_image = bg_rois_per_this_image - bg_highscor_this_image
    bg_random_this_image = max(bg_random_this_image, 0)
    #select false sample scores,but actually the scores and proposals are score descended
	
    bg_inds_high_score = nonfg_inds[:bg_highscor_this_image]

    # Sample background regions without replacement
    if bg_inds.size > 0:
        bg_inds_random = npr.choice(bg_inds, size=bg_random_this_image, replace=False)

    #bg_inds might duplicate due to random pick
    bg_inds = np.hstack((bg_inds_high_score,bg_inds_random))
